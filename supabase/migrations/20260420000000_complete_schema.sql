...
CREATE TABLE IF NOT EXISTS public.transactions (
  ...
  description text,
  reference   text,   -- ex: match_id, tournament_id
...
CREATE POLICY "Players see own matches" ON public.matches FOR SELECT TO authenticated
  USING (player1_id = auth.uid() OR player2_id = auth.uid());
CREATE POLICY "Insert match"            ON public.matches FOR INSERT TO authenticated WITH CHECK (player1_id = auth.uid());
CREATE POLICY "Update own match"        ON public.matches FOR UPDATE TO authenticated
  USING (player1_id = auth.uid() OR player2_id = auth.uid());
...
CREATE TABLE IF NOT EXISTS public.tournaments (
  ...
  status              text NOT NULL DEFAULT 'draft'
    CHECK (status IN ('draft','registering','starting','live','finished','cancelled')),
...
CREATE TABLE IF NOT EXISTS public.tournament_registrations (
  ...
  status        text NOT NULL DEFAULT 'registered'
    CHECK (status IN ('registered','playing','eliminated','winner','refunded')),
...
-- View: contagem de inscritos por torneio
...
-- ── TABELA: game_sessions (slots/crash/mines/plinko) ──────────
...
-- ── Função: registrar partida e atualizar perfil ──────────────
...
CREATE OR REPLACE FUNCTION public.register_tournament(
...
BEGIN
  ...
  -- Verificar saldo
  IF _t.entry_wallet = 'cash' AND _profile.cash_balance < _t.entry_fee THEN
    RETURN '{"error":"Saldo insuficiente"}'::jsonb;
  END IF;
  IF _t.entry_wallet = 'credits' AND _profile.credits_balance < _t.entry_fee THEN
    RETURN '{"error":"Créditos insuficientes"}'::jsonb;
  END IF;

  -- Verificar vagas
  SELECT COUNT(*) INTO _count FROM public.tournament_registrations
  WHERE tournament_id = p_tournament_id AND status != 'refunded';
...
-- ── Torneios de exemplo ───────────────────────────────────────
INSERT INTO public.tournaments (name, description, game_id, game_icon, entry_fee, max_players, guaranteed_prize, starts_at, status)
VALUES
